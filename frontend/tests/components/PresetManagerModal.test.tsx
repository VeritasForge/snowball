...
describe("PresetManagerModal — apply flow + dry-run", () => {
  ...
  it("[Boundary] dry-run: empty-string ITEM code is code-less → name-matches any asset (agrees with backend)", () => {
    const acc = makeAccount({ assets: [asset({ id: 1, name: "삼성전자", code: "005930" })] });
    const p = preset({ id: 7, name: "EI", items: [
      { name: "삼성전자", code: "", category: "주식", target_weight: 100 },  // empty item code = code-less
    ] });
    mockUsePresets.mockReturnValue(hook({ presets: [p] }));
    renderModal({ account: acc });
    fireEvent.click(screen.getByRole("button", { name: "적용" }));
